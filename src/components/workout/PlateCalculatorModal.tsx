...
import { X, Dumbbell, RotateCcw, Check, Plus, Minus, Sparkles } from 'lucide-react';
...
const AVAILABLE_PLATES: PlateOption[] = [
  { weight: 25, color: 'bg-red-600', borderColor: 'border-red-400', textColor: 'text-white', label: '25 kg', heightPx: 80, widthPx: 14 },
  { weight: 20, color: 'bg-blue-600', borderColor: 'border-blue-400', textColor: 'text-white', label: '20 kg', heightPx: 76, widthPx: 13 },
  { weight: 15, color: 'bg-amber-500', borderColor: 'border-amber-300', textColor: 'text-slate-950', label: '15 kg', heightPx: 70, widthPx: 12 },
  { weight: 10, color: 'bg-emerald-600', borderColor: 'border-emerald-400', textColor: 'text-white', label: '10 kg', heightPx: 62, widthPx: 11 },
  { weight: 5, color: 'bg-slate-200', borderColor: 'border-white', textColor: 'text-slate-900', label: '5 kg', heightPx: 52, widthPx: 10 },
  { weight: 2.5, color: 'bg-slate-800', borderColor: 'border-slate-500', textColor: 'text-slate-200', label: '2.5 kg', heightPx: 44, widthPx: 8 },
  { weight: 1.25, color: 'bg-zinc-400', borderColor: 'border-zinc-200', textColor: 'text-zinc-950', label: '1.25 kg', heightPx: 38, widthPx: 7 },
  { weight: 0.5, color: 'bg-purple-600', borderColor: 'border-purple-300', textColor: 'text-white', label: '0.5 kg', heightPx: 32, widthPx: 6 },
];
...
export const PlateCalculatorModal: React.FC<PlateCalculatorModalProps> = ({
  ...
  // Auto-Calculate Plates per side from Target Weight
  const calculatedPlatesPerSide = useMemo(() => {
    if (calcMode === 'manual') {
      return manualPlatesPerSide;
    }

    const availableWeight = (targetWeight - actualBarWeight - collarWeight * 2);
    ...
    return plates;
  }, [targetWeight, actualBarWeight, collarWeight, calcMode, manualPlatesPerSide]);
  ...
  const handleAddManualPlate = (plateWeight: number) => {
    setCalcMode('manual');
    setManualPlatesPerSide(prev => [...prev, plateWeight].sort((a, b) => b - a));
  };

  const handleRemoveManualPlate = (index: number) => {
    setCalcMode('manual');
    setManualPlatesPerSide(prev => prev.filter((_, i) => i !== index));
  };

  const handleResetPlates = () => {
    setManualPlatesPerSide([]);
    setTargetWeight(actualBarWeight);
  };
  ...
  return createPortal(
    <div 
      ...
      className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fade-in overflow-hidden"
    >
      <div 
      ...
      >
        ...
        <div className="flex items-center justify-between pb-3 border-b border-white/10 shrink-0 relative z-10">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold shrink-0 shadow-lg ${
              activeTab === 'Barbell' 
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-400 font-mono truncate">Precision load telemetry & plate breakdown</p>
            </div>
          </div>
          <button
            type="button"
        ...
        {/* Modal Scrollable Body (Strictly overflow-x-hidden, full width bounds) */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-3.5 pr-1 text-slate-200 relative z-10 w-full min-w-0">
          {activeTab === 'Barbell' ? (
            <>
              ...
              <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-br from-[#0D1829] via-[#0A1220] to-[#080E18] border border-emerald-500/30 flex items-center justify-between shadow-inner w-full min-w-0">
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-slate-300 font-mono mt-0.5 truncate">
                    {actualBarWeight}kg Bar + {calculatedPlatesPerSide.reduce((s, p) => s + p, 0)}kg plates × 2
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0 pl-2">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white text-[11px] font-mono flex items-center gap-1 border border-white/10 transition-colors"
                  >
              ...
              {/* Realistic Responsive Barbell Graphic Visualizer */}
              <div className="p-3 rounded-2xl bg-[#060912] border border-white/10 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl w-full min-w-0">
                <div className="text-[9px] font-mono text-slate-400 uppercase tracking-widest text-center w-full mb-1">
                  Barbell Sleeve • Plates Loaded Per Side ({calculatedPlatesPerSide.length} plates)
                </div>
                ...
                <div className="w-full flex items-center justify-center py-2 overflow-hidden">
                  <div className="flex items-center max-w-full justify-center">
                    {/* Center Shaft */}
                    <div className="w-10 sm:w-16 h-3 bg-gradient-to-b from-zinc-400 via-zinc-200 to-zinc-500 rounded-l shadow-md border-y border-zinc-600 flex items-center justify-center shrink-0">
                      <span className="text-[6px] font-mono text-zinc-800 font-black opacity-60">KNURL</span>
                    </div>

                    {/* Inside Stopper Ring */}
                    <div className="w-2.5 h-10 bg-gradient-to-r from-zinc-300 to-zinc-400 rounded-sm border border-zinc-600 shadow-lg shrink-0" />

                    {/* Sleeve (Stacked Plates) */}
                    <div className="h-5 min-w-[70px] max-w-[170px] bg-gradient-to-b from-zinc-300 via-zinc-100 to-zinc-400 border-y border-zinc-500 flex items-center px-0.5 relative shadow-inner">
                          ...
                          {calculatedPlatesPerSide.map((plateWeight, idx) => {
                            ...
                            return (
                              <button
                                ...
                                style={{ height: `${Math.min(pData.heightPx, 68)}px`, width: `${Math.max(pData.widthPx - 3, 7)}px` }}
                                className={`${pData.color} ${pData.borderColor} rounded-sm border shadow-md flex flex-col items-center justify-center text-[7px] font-mono font-black ${pData.textColor} hover:scale-105 hover:brightness-125 transition-transform pressable cursor-pointer shrink-0`}
                              >
                                <span className="rotate-90 select-none leading-none whitespace-nowrap">
                                  {pData.weight}
                                </span>
                              </button>
                            );
                    ...
                    {/* Bar End Cap */}
                    <div className="w-2.5 h-6 bg-gradient-to-r from-zinc-400 to-zinc-600 rounded-r border-y border-r border-zinc-700 shadow shrink-0 flex items-center justify-center">
                ...
                {calculatedPlatesPerSide.length > 0 && (
                  <div className="flex items-center gap-1.5 flex-wrap justify-center mt-1 pt-1.5 border-t border-white/5 w-full">
                    <span className="text-[10px] font-mono text-slate-400">Each Side:</span>
                    {Object.entries(plateCountSummary).map(([wt, count]) => {
                      const p = AVAILABLE_PLATES.find(item => item.weight === Number(wt));
                      return (
                        <span
                          key={wt}
                          className={`px-2 py-0.5 rounded-md text-[10px] font-mono font-bold border ${p?.color || 'bg-slate-800'} ${p?.textColor || 'text-white'} ${p?.borderColor || 'border-white/20'}`}
                        >
                          {count} × {wt}kg
                        </span>
                      );
              ...
              {/* 2. Target Total Weight Stepper (With Guaranteed Visible + Button and No Side Scroll) */}
              <div className="space-y-2 pt-2 border-t border-white/10 w-full min-w-0">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-mono uppercase text-slate-300 font-bold">
                    2. Target Total Weight
                  </label>
                  <div className="flex gap-1">
                    <button
                      type="button"
                      onClick={() => setCalcMode('target')}
                      className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${calcMode === 'target' ? 'bg-emerald-500 text-slate-950 font-black' : 'bg-white/5 text-slate-400'}`}
                    >
                      Target Mode
                    </button>
                    <button
                      ...
                      className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${calcMode === 'manual' ? 'bg-emerald-500 text-slate-950 font-black' : 'bg-white/5 text-slate-400'}`}
                    >
                      Rack Stacker
                    </button>
                  </div>
                ...
                {/* Target Weight Slider & Stepper - Full Width, Clear Minus and Plus Buttons */}
                <div className="flex items-center gap-2 bg-[#0E1424] p-1.5 sm:p-2 rounded-2xl border border-white/10 w-full min-w-0">
                  <button
                    type="button"
                    onClick={() => {
                      setCalcMode('target');
                      setTargetWeight(prev => Math.max(actualBarWeight, prev - 2.5));
                    }}
                  ...
                  <div className="flex-1 min-w-0 relative">
                    <input
                      ...
                      onChange={(e) => {
                        setCalcMode('target');
                        setTargetWeight(parseFloat(e.target.value) || actualBarWeight);
                      }}
                  ...
                  {[40, 50, 60, 70, 80, 90, 100, 110, 120, 140, 160, 180, 200].map(wt => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-mono font-bold transition-all border ${
                        targetWeight === wt && calcMode === 'target'
                ...
                <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5 w-full min-w-0">
                  {AVAILABLE_PLATES.map((p) => (
                    <button
                      ...
                      className={`p-2 rounded-xl flex flex-col items-center justify-center border transition-all pressable ${p.color} ${p.borderColor} ${p.textColor} shadow-md hover:scale-105 active:scale-95`}
                    >
                      <span className="text-xs font-black font-mono leading-none">{p.weight}</span>
                      <span className="text-[9px] opacity-80 mt-0.5">kg</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <>
              ...
              <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-br from-[#0A1828] to-[#070E18] border border-cyan-500/30 flex items-center justify-between shadow-inner w-full min-w-0">
                ...
                <div className="flex flex-col gap-1.5 shrink-0 pl-2">
                  <button
                    type="button"
                    onClick={() => setDumbbellMode(m => m === 'pair' ? 'single' : 'pair')}
                    className="px-3 py-1.5 rounded-xl bg-cyan-500/20 text-cyan-300 text-xs font-mono font-bold border border-cyan-500/30 hover:bg-cyan-500/30 transition-colors"
                  >
              ...
              <div className="space-y-2 w-full min-w-0">
                ...
                <div className="flex items-center gap-2 bg-[#0E1424] p-1.5 sm:p-2 rounded-2xl border border-white/10 w-full min-w-0">
                  <button
                    type="button"
                    onClick={() => setDumbbellWeight(prev => Math.max(1, prev - 2.5))}
                  ...
                  <button
                    type="button"
                    onClick={() => setDumbbellWeight(prev => prev + 2.5)}
                    className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 active:bg-cyan-500/20 text-white shrink-0 pressable flex items-center justify-center border border-white/5 hover:border-cyan-500/30 transition-all"
                  >
